import {FriendsEpisodeData, FriendsSceneData, FriendsSeasonData, FriendsUtteranceData} from "@/lib/types";
import path from "path";
import {promises as fs} from "fs";
import {EPISODE_NAMES} from "@/lib/constants";

export async function loadFriendsEpisode(season: number, episode: number): Promise<FriendsEpisodeData> {
  ...
  const episodeData = seasonData.episodes.find((episodeData: FriendsEpisodeData) => episodeData.episode_id === episodeID);
  if (!episodeData) {
  ...
  return {
    ...
    scenes: episodeData.scenes.map(
      (scene: FriendsSceneData) => ({
        scene_id: scene.scene_id,
        utterances: scene.utterances.map(
